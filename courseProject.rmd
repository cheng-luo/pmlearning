...
```{r,cache=TRUE}
testData=read.csv("pml-testing.csv",stringsAsFactors = T,na.strings=c("",NA))

trainData=read.csv("pml-training.csv",stringsAsFactors = T,na.strings=c("",NA))
```
...
```{r,cache=TRUE}
set.seed(13825)
modfit=train(classe~.,data=trainData,method="rf",trControl=fitControl,verbose=F)
```
